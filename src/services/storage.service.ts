...
export class DatabaseStorage implements IStorage {
  ...
  // System method to update accountability thread userId (bypasses authorization)
  async updateAccountabilityThreadUserId(
    threadId: number,
    newUserId: number
  ): Promise<ForumThread> {
    const [thread] = await db
    ...
    // Update the thread's userId directly
    const [updated] = await db
      .update(forumThreads)
      .set({
        userId: newUserId,
...
> {
  ...
  // Onboarding Steps
  async getAllOnboardingSteps(): Promise<OnboardingStep[]> {
  ...
  async createOnboardingStep(step: InsertOnboardingStep): Promise<OnboardingStep> {
    // id is auto-generated by serial, so we don't need to include it
  ...
  async updateOnboardingStep(
    ...
    // Remove id from updates if present (shouldn't update the id)
