import { pgTable, text, serial, integer, boolean, timestamp, jsonb, varchar, index, uniqueIndex, unique, real } from "drizzle-orm/pg-core";
...
export const userLogins = pgTable("user_logins", {
  ...
  userLoginIdx: index("idx_user_logins_user_login").on(table.userId, table.loginAt),
}));
...
export const funnelCopies = pgTable("funnel_copies", {
  ...
  userOfferIdx: uniqueIndex("idx_funnel_copies_user_offer").on(table.userId, table.offerNumber),
}));
...
export const interviewProcessingJobs = pgTable("interview_processing_jobs", {
  ...
  transcriptIds: integer("transcript_ids").array().notNull().default(sql`ARRAY[]::integer[]`), // IDs of transcripts to process
  processedTranscriptIds: integer("processed_transcript_ids").array().notNull().default(sql`ARRAY[]::integer[]`), // IDs successfully processed
...
export const sectionCompletions = pgTable("section_completions", {
  ...
  userStepIdx: index("idx_section_completions_user_step").on(table.userId, table.stepNumber),
  uniqueCompletion: index("idx_section_completions_unique").on(table.userId, table.stepNumber, table.sectionTitle),
  uniqueOfferCompletion: index("idx_section_completions_offer_unique").on(table.userId, table.stepNumber, table.sectionTitle, table.offerNumber),
}));
...
export const checklistItems = pgTable("checklist_items", {
  ...
  userSectionIdx: index("idx_checklist_items_user_section").on(table.userId, table.sectionKey),
...
export const workbookResponses = pgTable("workbook_responses", {
  ...
  uniqueUserOfferNumberQuestion: uniqueIndex("unique_user_offer_number_question").on(table.userId, table.offerNumber, table.stepNumber, table.questionKey),
  // Enhanced unique constraint including offer_id for multi-offer support
  uniqueUserOfferQuestion: index("unique_user_offer_question").on(table.userId, table.offerId, table.stepNumber, table.questionKey),
  ...
  step4SalesStrategyIndex: index("step4_sales_strategy_idx").on(table.userId, table.stepNumber, table.sectionTitle),
  step4CustomerLocationsIndex: index("step4_customer_locations_idx").on(table.userId, table.questionKey),
  step4DailyPlanningIndex: index("step4_daily_planning_idx").on(table.stepNumber, table.sectionTitle, table.userId),
  // Multi-offer indexes enhanced with offer_number
  offerResponsesIndex: index("offer_responses_idx").on(table.offerId, table.stepNumber),
  offerNumberResponsesIndex: index("offer_number_responses_idx").on(table.userId, table.offerNumber, table.stepNumber),
}));
...
export const userOfferOutlines = pgTable("user_offer_outlines", {
  ...
  userOfferIndex: index("user_offer_outline_idx").on(table.userId, table.offerId),
  // Legacy index for backward compatibility
  userActiveIndex: index("user_active_outline_idx").on(table.userId, table.isActive),
  // New offer number index for independent offer support
  userOfferNumberIndex: index("user_offer_number_outline_idx").on(table.userId, table.offerNumber),
  // CRITICAL: Unique constraint to ensure only one active outline per user per offer number
  uniqueActiveOfferOutline: uniqueIndex("unique_active_offer_outline").on(table.userId, table.offerNumber, table.isActive).where(eq(table.isActive, true)),
}));
...
export const coreOfferCoachingSessions = pgTable("core_offer_coaching_sessions", {
  ...
  userQuestionIndex: index("core_coaching_user_question_idx").on(table.userId, table.questionKey),
  offerQuestionIndex: index("core_coaching_offer_question_idx").on(table.offerId, table.questionKey),
  statusIndex: index("core_coaching_status_idx").on(table.userId, table.status),
}));
...
export const optimizationSuggestions = pgTable("optimization_suggestions", {
  ...
  uniqueUserSuggestions: uniqueIndex("unique_user_suggestions").on(table.userId),
}));
...
export const generatedContentStrategies = pgTable("generated_content_strategies", {
  ...
  userIdIndex: index("generated_content_strategies_user_idx").on(table.userId),
  activeIndex: index("generated_content_strategies_active_idx").on(table.userId, table.isActive),
}));
...
export const liveLaunches = pgTable("live_launches", {
  ...
  userStatusIndex: index("live_launches_user_status_idx").on(table.userId, table.status),
}));
...
export const liveLaunchFunnelMetrics = pgTable("live_launch_funnel_metrics", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  liveLaunchId: integer("live_launch_id").references(() => liveLaunches.id).notNull(),
  ...
  launchDateMetricIndex: index("live_launch_funnel_metrics_launch_date_metric_idx").on(table.liveLaunchId, table.date, table.metricType),
  userLaunchIndex: index("live_launch_funnel_metrics_user_launch_idx").on(table.userId, table.liveLaunchId),
...
export const liveLaunchOrganicMetrics = pgTable("live_launch_organic_metrics", {
  ...
  launchDateMetricIndex: index("live_launch_organic_metrics_launch_date_metric_idx").on(table.liveLaunchId, table.date, table.metricType),
  userLaunchIndex: index("live_launch_organic_metrics_user_launch_idx").on(table.userId, table.liveLaunchId),
  uniqueMetric: unique().on(table.liveLaunchId, table.date, table.metricType),
}));
...
export const emailTracking = pgTable("email_tracking", {
  ...
  userDateIndex: index("email_tracking_user_date_idx").on(table.userId, table.date),
...
export const liveLaunchOptimizationSuggestions = pgTable("live_launch_optimization_suggestions", {
  ...
  launchIndex: index("live_launch_optimization_suggestions_launch_idx").on(table.liveLaunchId),
  userLaunchIndex: index("live_launch_optimization_suggestions_user_launch_idx").on(table.userId, table.liveLaunchId),
}));
...
export const launchRegistrationFunnelData = pgTable("launch_registration_funnel_data", {
  ...
  userIdIndex: uniqueIndex("launch_registration_funnel_data_user_idx").on(table.userId),
}));
...
export const igniteDocuments = pgTable("ignite_documents", {
  ...
  userTypeIndex: index("ignite_documents_user_type_idx").on(table.userId, table.docType),
}));
...
export const launchEmailSequences = pgTable("launch_email_sequences", {
  ...
  userTypeIndex: index("launch_email_sequences_user_type_idx").on(table.userId, table.emailType),
}));
...
export const videoScriptGeneratorState = pgTable("video_script_generator_state", {
  ...
  userIdIndex: index("video_script_generator_state_user_idx").on(table.userId),
}));
...
export const forumThreads = pgTable("forum_threads", {
  ...
  categoryActivityIndex: index("forum_threads_category_activity_idx").on(table.categoryId, table.lastActivityAt),
...
export const forumPosts = pgTable("forum_posts", {
  ...
  threadIndex: index("forum_posts_thread_idx").on(table.threadId, table.createdAt),
...
export const weeklyAccountabilityThreads = pgTable("weekly_accountability_threads", {
  ...
  weekStartIndex: index("weekly_accountability_week_start_idx").on(table.weekStartDate),
...
export const accountabilityThreadParticipation = pgTable("accountability_thread_participation", {
  ...
  weeklyUserIndex: uniqueIndex("accountability_participation_weekly_user_idx").on(table.weeklyThreadId, table.userId),
...
export const insertForumThreadSchema = createInsertSchema(forumThreads).omit({
  ...
  title: z.string().min(4, "Title must be at least 4 characters").max(140, "Title must be less than 140 characters"),
  body: z.string().min(10, "Thread content must be at least 10 characters").max(20000, "Thread content must be less than 20,000 characters"),
});
...
export const insertForumPostSchema = createInsertSchema(forumPosts).omit({
  ...
  body: z.string().min(10, "Reply must be at least 10 characters").max(20000, "Reply must be less than 20,000 characters"),
});
...
export type WeeklyAccountabilityThread = typeof weeklyAccountabilityThreads.$inferSelect;
export type AccountabilityThreadParticipation = typeof accountabilityThreadParticipation.$inferSelect;
...
export const signupUserSchema = insertUserSchema.pick({
  email: true,
  firstName: true,
  lastName: true,
}).extend({
...
export const insertSectionCompletionSchema = createInsertSchema(sectionCompletions).omit({
  id: true,
  completedAt: true,
  updatedAt: true,
});
...
export const insertInterviewProcessingJobSchema = createInsertSchema(interviewProcessingJobs).omit({
  createdAt: true,
  updatedAt: true,
});
...
export const insertCoreOfferCoachingSessionSchema = createInsertSchema(coreOfferCoachingSessions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertFunnelTrackerDataSchema = createInsertSchema(funnelTrackerData).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertOptimizationSuggestionsSchema = createInsertSchema(optimizationSuggestions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertContentStrategyResponseSchema = createInsertSchema(contentStrategyResponses).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertGeneratedContentStrategySchema = createInsertSchema(generatedContentStrategies).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertIgniteDocumentSchema = createInsertSchema(igniteDocuments).omit({
  id: true,
  createdAt: true,
});

export const insertVideoScriptGeneratorStateSchema = createInsertSchema(videoScriptGeneratorState).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertMessagingStrategySchema = createInsertSchema(messagingStrategies).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertStrategyInterviewLinkSchema = createInsertSchema(strategyInterviewLinks).omit({
  id: true,
  createdAt: true,
});
...
export type InterviewProcessingJob = typeof interviewProcessingJobs.$inferSelect;
export type InsertInterviewProcessingJob = z.infer<typeof insertInterviewProcessingJobSchema>;
...
export type CoreOfferCoachingSession = typeof coreOfferCoachingSessions.$inferSelect;
export type InsertCoreOfferCoachingSession = z.infer<typeof insertCoreOfferCoachingSessionSchema>;
...
export type InsertFunnelTrackerData = z.infer<typeof insertFunnelTrackerDataSchema>;
...
export type ContentStrategyResponse = typeof contentStrategyResponses.$inferSelect;
export type InsertContentStrategyResponse = z.infer<typeof insertContentStrategyResponseSchema>;
export type GeneratedContentStrategy = typeof generatedContentStrategies.$inferSelect;
export type InsertGeneratedContentStrategy = z.infer<typeof insertGeneratedContentStrategySchema>;
...
export type VideoScriptGeneratorState = typeof videoScriptGeneratorState.$inferSelect;
export type InsertVideoScriptGeneratorState = z.infer<typeof insertVideoScriptGeneratorStateSchema>;
...
export const insertLiveLaunchFunnelMetricSchema = createInsertSchema(liveLaunchFunnelMetrics).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertLiveLaunchOrganicMetricSchema = createInsertSchema(liveLaunchOrganicMetrics).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLiveLaunchOptimizationSuggestionSchema = createInsertSchema(liveLaunchOptimizationSuggestions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type LiveLaunchFunnelMetric = typeof liveLaunchFunnelMetrics.$inferSelect;
export type InsertLiveLaunchFunnelMetric = z.infer<typeof insertLiveLaunchFunnelMetricSchema>;
export type LiveLaunchOrganicMetric = typeof liveLaunchOrganicMetrics.$inferSelect;
export type InsertLiveLaunchOrganicMetric = z.infer<typeof insertLiveLaunchOrganicMetricSchema>;
export type EmailTracking = typeof emailTracking.$inferSelect;
export type InsertEmailTracking = z.infer<typeof insertEmailTrackingSchema>;
export type LiveLaunchOptimizationSuggestion = typeof liveLaunchOptimizationSuggestions.$inferSelect;
export type InsertLiveLaunchOptimizationSuggestion = z.infer<typeof insertLiveLaunchOptimizationSuggestionSchema>;

export const insertLaunchRegistrationFunnelDataSchema = createInsertSchema(launchRegistrationFunnelData).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type LaunchRegistrationFunnelData = typeof launchRegistrationFunnelData.$inferSelect;
export type InsertLaunchRegistrationFunnelData = z.infer<typeof insertLaunchRegistrationFunnelDataSchema>;

export const insertLaunchEmailSequenceSchema = createInsertSchema(launchEmailSequences).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type InsertLaunchEmailSequence = z.infer<typeof insertLaunchEmailSequenceSchema>;
...
export const implementationCheckboxes = pgTable("implementation_checkboxes", {
  ...
  userPageIdx: index("idx_implementation_checkboxes_user_page").on(table.userId, table.pageIdentifier),
}));

export const insertImplementationCheckboxSchema = createInsertSchema(implementationCheckboxes).omit({
  id: true,
  updatedAt: true,
});

export type ImplementationCheckbox = typeof implementationCheckboxes.$inferSelect;
export type InsertImplementationCheckbox = z.infer<typeof insertImplementationCheckboxSchema>;
...
export const insertPlatformResourceSchema = createInsertSchema(platformResources).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
...
export const insertChecklistStepDefinitionSchema = createInsertSchema(checklistStepDefinitions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type ChecklistStepDefinition = typeof checklistStepDefinitions.$inferSelect;
export type InsertChecklistStepDefinition = z.infer<typeof insertChecklistStepDefinitionSchema>;
...
export const coachingCallRecordings = pgTable("coaching_call_recordings", {
  ...
  timestamps: text("timestamps"), // short summary lines: [HH:MM:SS] Name - topic (for "Short summary" view)
...
export const onboardingSteps = pgTable("onboarding_steps", {
  ...
  color: varchar("color", { length: 50 }).notNull(), // 'blue', 'coral', 'orange', 'navy'
  order: integer("order").default(0).notNull(),
  buttonText: varchar("button_text", { length: 255 }),
  buttonLink: varchar("button_link", { length: 500 }),
  buttonAction: varchar("button_action", { length: 20 }), // 'link' or 'route'
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (table) => ({
  orderIndex: index("idx_onboarding_steps_order").on(table.order),
}));

// Base schema without refine (for creating update schema)
const baseOnboardingStepSchema = createInsertSchema(onboardingSteps, {
  title: z.string().min(1, "Title is required").max(500, "Title must be less than 500 characters"),
  description: z.string().min(1, "Description is required"),
  descriptor: z.string().optional().nullable(),
  color: z.enum(["blue", "coral", "orange", "navy"], {
    errorMap: () => ({ message: "Color must be one of: blue, coral, orange, navy" }),
  }),
  order: z.number().int().optional().default(0),
  buttonText: z.string().optional().nullable(),
  buttonLink: z.string().optional().nullable(),
  buttonAction: z.enum(["link", "route"]).optional().default("link"),
}).omit({
  id: true, // id is auto-generated by serial
  createdAt: true,
  updatedAt: true,
});

// Insert schema with validation
export const insertOnboardingStepSchema = baseOnboardingStepSchema.refine(
  (data) => {
    // If buttonText is provided, buttonLink must also be provided
    if (data.buttonText && data.buttonText.trim() !== "") {
      return data.buttonLink && data.buttonLink.trim() !== "";
    }
    return true;
  },
  {
    message: "buttonLink is required when buttonText is provided",
    path: ["buttonLink"],
  }
);

// Update schema with validation
export const updateOnboardingStepSchema = baseOnboardingStepSchema.partial().extend({
  id: z.number().int().min(1, "ID is required"),
}).refine(
  (data) => {
    // If buttonText is provided (and not empty), buttonLink must also be provided
    if (data.buttonText !== undefined && data.buttonText !== null && data.buttonText.trim() !== "") {
      return data.buttonLink !== undefined && data.buttonLink !== null && data.buttonLink.trim() !== "";
    }
    return true;
  },
  {
...
// Team Members table for "Meet The Team" section
export const teamMembers = pgTable("team_members", {
  ...
  stepNumber: integer("step_number").notNull(),
  order: integer("order").default(0).notNull(),
  backgroundColor: varchar("background_color", { length: 50 }).default("blue").notNull(),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (table) => ({
  orderIndex: index("idx_team_members_order").on(table.order),
}));

// Base schema for team members
const baseTeamMemberSchema = createInsertSchema(teamMembers, {
  name: z.string().min(2, "Name must be at least 2 characters"),
  title: z.string().min(3, "Title must be at least 3 characters"),
  description: z.string().min(10, "Description must be at least 10 characters"),
  vimeoId: z.string().regex(/^\d+\/[a-zA-Z0-9]+$/, "Vimeo ID must be in format 'videoId/hash'"),
  stepNumber: z.number().int().min(0, "Step number must be 0 or greater"),
  order: z.number().int().optional().default(0),
  backgroundColor: z.enum(["blue", "coral", "orange", "navy"], {
    errorMap: () => ({ message: "Background color must be one of: blue, coral, orange, navy" }),
...
// Insert schema
export const insertTeamMemberSchema = baseTeamMemberSchema;

// Update schema (all fields optional)
export const updateTeamMemberSchema = baseTeamMemberSchema.partial();
...
export const faqs = pgTable("faqs", {
  ...
  orderIndex: index("idx_faqs_order").on(table.order),
}));

// Base schema for FAQs
const baseFaqSchema = createInsertSchema(faqs, {
...
// Insert schema
export const insertFaqSchema = baseFaqSchema;
...
// Journey Steps table for "Your IGNITE Journey" section
export const journeySteps = pgTable("journey_steps", {
  ...
  orderIndex: index("idx_journey_steps_order").on(table.order),
}));

// Base schema for journey steps
const baseJourneyStepSchema = createInsertSchema(journeySteps, {
...
// Insert schema
export const insertJourneyStepSchema = baseJourneyStepSchema;

// Update schema (all fields optional)
export const updateJourneyStepSchema = baseJourneyStepSchema.partial();
...
export type UpdateJourneyStep = z.infer<typeof updateJourneyStepSchema>;

// Orientation Video table - single video for "Start Here: Your IGNITE Orientation" section
export const orientationVideos = pgTable("orientation_videos", {
  id: serial("id").primaryKey(),
  vimeoId: varchar("vimeo_id", { length: 255 }).notNull(),
  title: varchar("title", { length: 255 }).notNull(),
...
// Base schema for orientation videos
const baseOrientationVideoSchema = createInsertSchema(orientationVideos, {
  vimeoId: z.string().regex(/^[\d]+(\/[\w]+)?$/, "Vimeo ID must be in format 'videoId' or 'videoId/hash'"),
  title: z.string().min(3, "Title must be at least 3 characters").max(255, "Title must be less than 255 characters"),
  stepNumber: z.number().int().min(0).max(0, "Step number must be 0 for orientation video"),
}).omit({
  id: true, // id is auto-generated by serial
  createdAt: true,
  updatedAt: true,
...
// Insert schema
export const insertOrientationVideoSchema = baseOrientationVideoSchema;

// Update schema (all fields optional, but stepNumber must remain 0 if provided)
export const updateOrientationVideoSchema = baseOrientationVideoSchema.partial().refine(
  (data) => {
    // If stepNumber is provided, it must be 0
    if (data.stepNumber !== undefined && data.stepNumber !== 0) {
      return false;
    }
    return true;
  },
  {
    message: "Step number must be 0 for orientation video",
    path: ["stepNumber"],
  }
);
